...
const AddProduct = ({
    ...
    const handleAddNewEntry = async () => {
        try {
            if (addType === 'category') {
                const response = await $api.post(`http://localhost:5000/api/v1.0/product/type?action=add&subjet=category`, {
                    title: newEntry,
                });
                setCategoryList([...categoryList, response.data]);
                setInputCategId(response.data.id);
            } else if (addType === 'type') {
                const response = await $api.post(`http://localhost:5000/api/v1.0/product/type?action=add&subjet=type`, {
                    title: newEntry,
                });
