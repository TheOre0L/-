...
const Post = ({
  ...
  const [colorChoise, setColor] = useState({id: null, color: null});
  if (isLoading) {
    return <PostSkeleton />;
  }
  return (
          ...
          <>
            ...
            <div className={styles.wrapper}>
              ...
              <div className={styles.indention}>
                <h2 className={`${clsx(styles.title, { [styles.titleFull]: isFullPost })} m-3`}>
                  {isFullPost ? title : <Link to={`/product/${id}`}>{title}</Link>}
                </h2>
                <ul className={`${styles.tags} m-3`}>
                  <li key={type}>
                    {isPreview ? (
                      `${type}`
                    ) : (
                ...
                <Box>
                  <Card className={'2xl:w-12/12'} sx={{ marginTop: 5 }}>
                    ...
                    <Box>
                      ...
                      <Typography level="body-xs">Цвета (Выбран: {colorChoise.color})</Typography>
                        <li>
                          <Button.Group className="h-10 w-12/12">
                            {colors.map((color) => (
                              <Button onClick={() => {setColor({id: color.id, color: color.color})}} color="gray" key={color.id}>{color.color}</Button>
                            ))}
                          </Button.Group>
                        </li>
                      </ul>
                    ...
                    <CardContent orientation="horizontal">
                      ...
                      <ul className={styles.postDetails}>
                        <li className="-mt-10">
                          <Button.Group>
                            <Button color="blue" className="ml-2" disabled={colorChoise.color === null ? true : false} onClick={() => {addShop(id, store.user.id, colorChoise.color, store)}}>
